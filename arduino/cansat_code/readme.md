...
- Make sure there is a switch or something between the TX pin on GPS and RX / 0 pin on the Arduino, this switch needs to be off every time you upload code. It needs to be on while running the code however, but you can still use the serial monitor, use DEBUG_PORT instead of Serial.
